    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-4">
                ...
                <div class="flex-1">
                    <h1 class="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-orange-600">
                        Gestión de Promociones
                    </h1>
                    <p class="text-gray-500 mt-1">Crea, edita y administra todas tus ofertas y campañas.</p>
                </div>
                <!-- Create Button moved to header -->
                <button @click="openCreatePromoModal" class="px-5 py-2.5 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 text-white font-bold rounded-lg shadow-md hover:shadow-lg transition-all flex items-center gap-2">
        ...
        <div class="min-h-screen bg-gray-50 text-gray-800 p-8 font-sans pb-24">
                ...
                <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-md">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-sm whitespace-nowrap">
                            <thead class="uppercase tracking-wider border-b border-gray-200 bg-gray-50 text-gray-500 text-xs">
                                <tr>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <tr v-for="promo in promociones.data" :key="'p'+promo.id" class="hover:bg-gray-50 transition group">
                                    <td class="p-4 flex gap-2">
                                        <button @click="openEditPromoModal(promo)" class="text-blue-600 hover:text-blue-500 bg-blue-50 p-2 rounded-lg transition" title="Editar">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                        </button>
                                        <button @click="deletePromo(promo.id)" class="text-red-600 hover:text-red-500 bg-red-50 p-2 rounded-lg transition" title="Eliminar">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        </button>
                                    </td>
                                    <td class="p-4">
                                        <div class="flex items-center gap-3">
                                            <img v-if="promo.foto" :src="promo.foto" class="w-10 h-10 rounded border border-gray-200 object-cover" />
                                            <div v-else class="w-10 h-10 rounded border border-gray-200 bg-gray-100 flex items-center justify-center text-xs text-gray-400">IMG</div>
                                            <div>
                                                <p class="font-bold text-gray-800">{{ promo.nombre }} <span class="text-gray-400 text-xs font-mono ml-2">(ID:{{ promo.id }})</span></p>
                                                <span class="px-2 py-0.5 mt-1 inline-block text-[10px] font-bold uppercase rounded bg-orange-100 text-orange-600 border border-orange-200">
                                                    {{ promo.tipo }}
                                                </span>
                                            </div>
                                        </div>
                                    ...
                                    <td class="p-4 text-right">
                                        <span v-if="promo.promo_price" class="font-bold text-gray-800 text-lg">${{ parseFloat(promo.promo_price).toFixed(2) }}</span>
                    ...
                    <div class="p-4 border-t border-gray-200 flex items-center justify-between text-sm text-gray-500 bg-gray-50/50">
                        ...
                        <div class="flex space-x-1">
                            <component v-for="(link, i) in promociones.links" :key="i" :is="link.url ? Link : 'span'" :href="link.url"
                                ...
                                :class="[link.active ? 'bg-orange-600 border-orange-500 text-white shadow-sm' : 'bg-white border-gray-300 text-gray-600 hover:bg-gray-50', !link.url && 'opacity-50 cursor-not-allowed']"
                                v-html="link.label">
                            </component>
                        </div>
                    </div>
            ...
            <header class="mb-6 flex justify-between items-center border-t border-gray-200 pt-10">
                <div>
                    <h2 class="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">
                        Productos en Promoción
                    </h2>
                    <p class="text-gray-500 mt-1">Vincula productos a tus promociones y dales MV Coins extra.</p>
                </div>
                <button @click="openCreateProdPromoModal" class="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold rounded-lg shadow-md hover:shadow-lg transition-all flex items-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                    Vincular Producto
                </button>
            </header>
                ...
                <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-md">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-sm whitespace-nowrap">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <tr v-for="pp in productosPromociones.data" :key="'pp'+pp.id" class="hover:bg-gray-50 transition group">
                                    <td class="p-4 flex gap-2">
                                        <button @click="openEditProdPromoModal(pp)" class="text-indigo-600 hover:text-indigo-500 bg-indigo-50 p-2 rounded-lg transition" title="Editar">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                        </button>
                                        <button @click="deleteProdPromo(pp.id)" class="text-red-600 hover:text-red-500 bg-red-50 p-2 rounded-lg transition" title="Eliminar">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        </button>
                                    </td>
                                    <td class="p-4">
                                        <div v-if="pp.promocion">
                                            <p class="font-bold text-gray-800">{{ pp.promocion.nombre }}</p>
                                            <p class="text-xs text-gray-400 mt-0.5">ID Promo: {{ pp.id_promocion }}</p>
                                        </div>
                                        <div v-else class="text-red-500 font-mono text-xs">ID Promoción: {{ pp.id_promocion }} (Huérfana)</div>
                                    </td>
                                    <td class="p-4">
                                        <div v-if="pp.producto">
                                            <p class="font-medium text-gray-800">{{ pp.producto.nombre }}</p>
                                            <p class="text-xs text-indigo-500 font-mono mt-0.5">SKU: {{ pp.producto.sku || pp.id_producto }}</p>
                                        </div>
                                        <div v-else class="font-mono text-xs text-gray-400">ID Producto: {{ pp.id_producto }}</div>
                                    </td>
                                    <td class="p-4 text-right">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-bold bg-yellow-100 text-yellow-700 border border-yellow-200">
                                            +{{ pp.mvcoins_extra }}
                                        </span>
                                    </td>
                                </tr>
                    ...
                    <div class="p-4 border-t border-gray-200 flex items-center justify-between text-sm text-gray-500 bg-gray-50/50">
                        ...
                        <div class="flex space-x-1">
                            <component v-for="(link, i) in productosPromociones.links" :key="i" :is="link.url ? Link : 'span'" :href="link.url"
                                class="px-3 py-1.5 rounded text-sm transition border"
                                :class="[link.active ? 'bg-indigo-600 border-indigo-500 text-white shadow-sm' : 'bg-white border-gray-300 text-gray-600 hover:bg-gray-50', !link.url && 'opacity-50 cursor-not-allowed']"
                                v-html="link.label">
                            </component>
                        </div>
                    </div>
            ...
            <div class="bg-gray-800 border border-gray-700 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <form @submit.prevent="submitPromoForm" class="flex-1 overflow-y-auto custom-scrollbar p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="space-y-1">
                            <label class="block text-xs font-medium text-gray-400 uppercase">Porcentaje Promocional</label>
